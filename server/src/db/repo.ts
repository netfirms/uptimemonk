...
export function insertFeedback(input: {
  ...
  getDb()
    .prepare(
      `INSERT INTO feedback
         (id, created_at, kind, message, email, name, uid, org_id, source, app_version)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      id,
  ...
  return rowToFeedback(
    getDb().prepare("SELECT * FROM feedback WHERE id = ?").get(id)
  );
}
 ...
 *
 * A double-submitted form is the common case — a slow network, an impatient
 * second click — and storing it twice makes an operator read it twice and
 * possibly reply twice. Compares the message itself rather than a request id
 * because the second submission is a genuinely separate request.
 */
export function isDuplicateFeedback(
  ...
  const r: any = getDb()
    ...
    .get(email, message, Date.now() - withinMs);
  return !!r;
}

export function listFeedback(opts: { status?: FeedbackStatus; limit?: number } = {}): Feedback[] {
  ...
  const rows = opts.status
    ...
    : getDb()
        .prepare(
          // Unhandled first, then newest — the order the console reads in.
...
export function countFeedbackByStatus(): Record<FeedbackStatus, number> {
  ...
  for (const r of rows) {
    if (r.status in out) out[r.status as FeedbackStatus] = r.n;
  }
  return out;
}

/** Returns false when the id does not exist, so the route can 404 honestly. */
