...
import { ALERT_FROM_EMAIL, APP_URL, USER_AGENT } from "../config";
import { assertSafeUrl } from "../lib/targetGuard";
import type { AlertContact, Incident, Monitor } from "../types";
        ...
        elements: [
          {
            ...
            url: `${APP_URL.value()}/monitors/${p.monitorId}`,
          },
        ],
      },
    ],
...
export async function sendWebhook(url: string, p: AlertPayload): Promise<void> {
  await postJsonToUserUrl(url, {
    ...
    incident: {
      id: p.incidentId,
      cause: p.incident.cause,
      startedAt: p.incident.startedAt.toDate().toISOString(),
      resolvedAt: p.incident.resolvedAt?.toDate().toISOString() ?? null,
...
export async function deliver(
  ...
  switch (contact.channel) {
    ...
    case "push":
      // FCM: see alerts/push.ts — kept out of the hot path for brevity.
      logger.info("push alerts not wired up yet", { contact: contact.name });
      return;
    default:
